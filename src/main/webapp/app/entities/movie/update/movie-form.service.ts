...
type MovieFormDefaults = Pick<NewMovie, 'id' | 'rights' | 'lefts'>;
...
type MovieFormGroupContent = {
  ...
  rights: FormControl<IMovie['rights']>;
  lefts: FormControl<IMovie['lefts']>;
};
...
export class MovieFormService {
  createMovieFormGroup(movie: MovieFormGroupInput = { id: null }): MovieFormGroup {
    ...
    return new FormGroup<MovieFormGroupContent>({
      ...
      rights: new FormControl(movieRawValue.rights ?? []),
      lefts: new FormControl(movieRawValue.lefts ?? []),
    });
  }
